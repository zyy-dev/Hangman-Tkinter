...
class Keyboard(CTkFrame):
    def __init__ (self, parent: object, guess: object, player_state: object, main_tk: object, character: str, character_object: object, time_callback, main_menu_callback, choose_callback: object) -> None:
    ...
    def clicked(self, btn: object) -> None:
        ...
        if not self.guess.validate_char(char):
            ...
            if self.mistakes > 5:
                ...
                self.disabled()
                self.after(2000, lambda: game_over(self.main_tk, self, self.time_callback, self.points, self.character, self.guess, self.main_menu_callback, self.character_object, self.choose_callback))
            else:
                # if wrong
                play_audio.wrong()
